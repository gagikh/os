...
int main()
{
	...
	while (true) {
		...
		WaitForSingleObject(x0, INFINITE); // take left fork
		WaitForSingleObject(x1, INFINITE); // take right fork
		std::cout << "1 - start eating" << std::endl;
		Sleep(10); // eat

		ReleaseMutex(x0); // take down
		ReleaseMutex(x1); // take down
		std::cout << "0 - start thinking" << std::endl;
